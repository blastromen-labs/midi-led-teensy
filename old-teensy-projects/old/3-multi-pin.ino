...
void updateGroupLeds(int group)
{
    ...
    // Adjust for the second panel
    int stripIndex = 0;
    if (group >= 4)
    {
        stripIndex = 2;  // Use the third output for the second panel
        startLed -= 256; // Adjust the LED index for the second panel
        endLed -= 256;
    }

    for (int i = startLed; i <= endLed; i++)
    {
        leds.setPixel(i + stripIndex * ledsPerStrip, r, g, b); // Note: RGB order for WS2811
    }
...
{
    if (pitch >= 120 && pitch <= 127)
    { // Adjust pitch range for 8 groups
        int group = 127 - pitch;
